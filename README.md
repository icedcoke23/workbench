...
- **智能待办**：自动根据即将到来的课次与未完成的反馈生成待办，支持看板拖拽
- **反馈模板系统**：4 个内置模板（通用 / 表扬 / 建议 / 进度）+ 自定义模板，支持变量占位符
- **数据备份**：一键导出全部 14 张表为 JSON，事务性导入（反向依赖删除 + 顺序插入）
- **文件同步**：基于 last-write-wins 策略的设备间数据同步
...
│   │   ├── database/
│   │   │   ├── db.ts           # 连接管理、迁移、种子
│   │   │   ├── schema.sql      # 14 张表 + 索引 + 触发器
│   │   │   ├── seed.ts         # 内置数据初始化
│   │   │   └── repositories/   # 13 个仓储模块
│   │   ├── services/           # 业务服务
│   │   │   ├── ai.ts           # AI 调用（课表解析、反馈生成）
│   │   │   ├── data.ts         # 数据备份/恢复
│   │   │   ├── feedback.ts     # 反馈生成与发送
│   │   │   ├── file.ts         # 文件操作（头像、资源、图片选择）
│   │   │   ├── history.ts      # 学生历史聚合
│   │   │   ├── logger.ts       # 文件日志 + 轮转
│   │   │   ├── pdf.ts          # PDF 导出
...
> `postinstall` 脚本会自动执行 `electron-rebuild` 重编译 better-sqlite3。若失败会打印 `rebuild skipped`，可手动执行 `npm run rebuild`。

### 3. 准备 Scratch GUI（可选，仅授课中心需要）

```bash
# 克隆并构建 Scratch GUI 到 ./scratch-gui 目录
...
| `npm run build` | 构建主进程 + preload + 渲染进程产物到 `out/` |
| `npm run preview` | 预览构建产物 |
| `npm run typecheck` | 全量类型检查（node + web） |
| `npm run lint` | ESLint 检查 |
| `npm run lint:fix` | ESLint 自动修复 |
| `npm run icon` | 从 `build/icon.svg` 重新生成 ICO 和 PNG |
| `npm run rebuild` | 重编译 better-sqlite3 原生模块 |
| `npm run build:win` | 构建并打包 Windows x64 安装包到 `release/` |
| `npm run build:dir` | 构建并打包为目录（免安装，用于测试） |

---

## 数据模型

数据库使用 SQLite（WAL 模式），共 14 张表：
...
const feedback = await call(window.api.feedback.generate({ classId, periodStart, periodEnd }))
```
...
设置 → 数据备份 → 立即导出，将所有 14 张表导出为 JSON 文件：
...
- `vue/block-order`: warn（推荐 script → template → style 顺序）
...
A: 原生模块未正确编译。执行 `npm run rebuild` 重新为 Electron 编译。
...
A: 需要在设置中配置 AI 接口（API Key、Base URL、模型名）。支持 OpenAI 兼容接口。
